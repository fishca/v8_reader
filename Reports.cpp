...
__fastcall TReports::TReports(v8catalog *_parent, const String& _guid, const String& _name)
{
	...
	node_att = &(*node_att)[0][4][1]; // количество элементов
	...
	for (int i = 0; i < CountAtt; i++)
	{
		try {
			...
			node_att_att = &(*node_att_att)[0][4][i+CountAtt-Delta][0][1][1][1][2];
	...
	node_att_t = &(*node_att_t)[0][6][1]; // количество элементов
	...
	for (int i = 0; i < CountAttTab; i++)
	{
		...
		node_att_tab = &(*node_att_tab)[0][6][i+CountAttTab-DeltaTab][0][1][5][1][2];
	...
	while (curNodeChild)
	{
		...
		if (curNodeChild)
		{
			String NameForm = GetNameFormReports(parent, curNodeChild->get_value());
	...
	node_att_c = &(*node_att_c)[0][7][1]; // количество элементов
	...
	for (int i = 0; i < CountCom; i++)
	{
		...
		node_com = &(*node_com)[0][7][i+CountCom-DeltaCom][0][1][2][9][2];
