...
//	unsigned int version_edit; // версия изменения
	...
	void set_len(unsigned __int64 _len); // установка новой длины объекта
...
public:
	...
	char* getdata(void* buf, unsigned __int64 _start, unsigned __int64 _length); // чтение кусочка объекта, поддерживает кеширование блоков. Буфер не принадлежит объекту
	bool setdata(void* buf, unsigned __int64 _start, unsigned __int64 _length); // запись кусочка объекта, поддерживает кеширование блоков.
	bool setdata(void* buf, unsigned __int64 _length); // запись объекта целиком, поддерживает кеширование блоков.
	bool setdata(TStream* stream); // записывает поток целиком в объект, поддерживает кеширование блоков.
	bool setdata(TStream* stream, unsigned __int64 _start, unsigned __int64 _length); // запись части потока в объект, поддерживает кеширование блоков.
	unsigned __int64 getlen();
	//void savetofile();
	void savetofile(String filename);
	void set_lockinmemory(bool _lock);
	static void garbage();
	unsigned __int64 get_fileoffset(unsigned __int64 offset); // получить физическое смещение в файле по смещению в объекте
...
private:
...
#ifndef PublicRelease
	void dump_recursive(v8object* file_index, TFileStream* f, int level, unsigned __int64 curblock);
	...
	void delete_index_record(const char* index_buf, const unsigned int phys_numrec, unsigned __int64 block, bool& is_last_record, bool& page_is_empty, char* new_last_index_buf, unsigned int& new_last_phys_num); // рекурсивное удаление одного индекса из блока файла index
	...
	void write_index_record(const unsigned int phys_numrecord, const char* index_buf, unsigned __int64 block, int& result, char* new_last_index_buf, unsigned int& new_last_phys_num, char* new_last_index_buf2, unsigned int& new_last_phys_num2, unsigned __int64& new_last_block2); // рекурсивная запись индекса
#endif //#ifdef PublicRelease
...
public:
	...
	char* unpack_leafpage(unsigned __int64 page_offset, unsigned int& number_indexes);
...
public:
	...
	__int64 get_fileoffset(unsigned int phys_numrecord); // получить физическое смещение в файле записи по номеру
	...
	unsigned int get_phys_numrec(int ARow, index* cur_index); // получить физический индекс записи по номеру строки по указанному индексу
...
public:
	...
	virtual int __fastcall Write(const ByteArr Buffer, int Offset, int Count){throw(Exception(L"Write read-only stream"));};
