...
bool ConfigStorageCFFile::fileexists(const String& path)
{
    ...
    // Но метод fileexists используется только для внешних файлов,
...
bool container_file::open()
{
	...
	if(maxpartno > 0)
    	stream = new TTempStream;
	else
    	stream = new TMemoryStream;

	if(packed == tfp_unknown)
    	packed = isPacked() ? tfp_yes : tfp_no;
...
bool container_file::ropen()
{
	...
	if(maxpartno > 0)
    	rstream = new TTempStream;
	else
    	rstream = new TMemoryStream;
...
bool container_file::isPacked()
{
	...
	if(name.CompareIC(L"DynamicallyUpdated") == 0)
    	return false;

	if(name.CompareIC(L"SprScndInfo") == 0)
    	return false;

	if(name.CompareIC(L"DBNamesVersion") == 0)
    	return false;

	if(name.CompareIC(L"siVersions") == 0)
    	return false;

	if(name.UpperCase().Pos(L"FO_VERSION") > 0)
    	return false;
	...
	if(i)
	{
		...
		if(ext.Compare(L"INF") == 0)
        	return false;

		if(ext.Compare(L"PFL") == 0)
        	return false;
	}
	return true;
...
bool ConfigStorageTable::fileexists(const String& path)
{
	...
	// Это неправильно для формата 8.0 с файлом каталогом metadata.
    // Но метод fileexists используется только для внешних файлов, поэтому такой проверки достаточно
...
ConfigStorageTableConfigCas::ConfigStorageTableConfigCas(TableFiles* tabc, const String& configver, T_1CD* _base) : ConfigStorageTable(_base)
{
	...
	for(m = ct->get_value().ToIntDef(0); m; --m)
	{
		...
		stream->Seek(0l, soBeginning);
		{
			v8reader::vcl_bridge::TStreamByteStreamAdapter streamAdapter(stream);
			std::u16string encoded = v8reader::vcl_bridge::StringToUtf16(ct->get_value());
			v8reader::core::encoding::base64_decode(encoded, streamAdapter);
		}
		stream->Seek(0l, soBeginning);
